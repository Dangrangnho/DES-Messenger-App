                            ...
                            if len(self.pending_files[file_name]['chunks']) == total_chunks:
                                ...
                                for i in range(1, total_chunks + 1):
                                    if i in self.pending_files[file_name]['chunks']:
                                        ...
                                        complete_hex += chunk_hex
                                        self._log("RECV", f"  [{i}] Giải mã ({dt:.1f}ms)")
                                
                                try:
                                    raw_bytes = bytes.fromhex(complete_hex)
                                    # Giải nén dữ liệu (sender đã nén trước khi mã hóa)
                                    file_bytes = zlib.decompress(raw_bytes)
